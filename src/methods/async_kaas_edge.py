"""
AsyncKaaSEdge: Asynchronous variant of KaaSEdge (JPDC 2026).
==============================================================

Differences from the synchronous KaaSEdge (IEEE EDGE 2026):

1. After RADS scheduling, applies StragglerModel to simulate latencies.
2. Uses a TimeoutPolicy to determine the per-round deadline D^(t).
3. Only collects logits from devices that are COMPLETE (+ PARTIAL if
   the policy accepts partial uploads).
4. Uses v_received (not v_star) for quality weighting.
5. Tracks wall-clock time (cumulative deadline sum).
6. Stores latency history for the adaptive timeout policy.

The RADS scheduling (water-filling + greedy), local training, logit
computation, LDP noise, and KL distillation are **identical** to the
synchronous version.
"""
...
class AsyncKaaSEdgeConfig(KaaSEdgeConfig):
    """Config for the async variant.  Inherits all sync parameters."""
    # ── Straggler model ──
    ...
    # ── Straggler-aware RADS ──
    straggler_aware: bool = True       # multiply greedy gain by pi_i(D)


# =========================================================================
# Async baselines that also live in this file
# =========================================================================

class AsyncKaaSEdge(FederatedMethod):
    """
    Asynchronous KaaS-Edge with straggler tolerance.

    Round protocol
    --------------
    1. Get deadline  D^(t) from timeout policy.
    2. RADS scheduling (with optional straggler-aware flag):
       - Stage 1: water-filling  →  {v_i*}          (unchanged)
       - Stage 2: greedy, gain *= pi_i(D)            (if straggler_aware)
    3. Simulate device latencies via StragglerModel.
    4. Filter by outcome:
       - complete  →  use v_star logits
       - partial   →  use v_received (only if policy.accepts_partial)
       - timeout   →  discard
    5. Collect logits on public ref set (identical to sync).
    6. Quality-weighted aggregation with v_received-based quality.
    ...
    def __init__(
        ...
        config: Optional[AsyncKaaSEdgeConfig] = None,
        n_classes: int = 100,
        ...
        self.config = config or AsyncKaaSEdgeConfig()
        ...
        # ── RADS scheduler (with straggler-aware flag) ──
...
class FullAsyncFD(AsyncKaaSEdge):
    """
    ...
    def __init__(
        ...
        cfg = config or AsyncKaaSEdgeConfig()
    ...
    def run_round(self, round_idx, devices, client_loaders, public_loader,
                  test_loader=None):
        # Override v_max so RADS gives everyone ~v_fixed
